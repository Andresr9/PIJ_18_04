    ...
    if validData
       ...
       if numBodies > 0
            ...
            %pos2D = k2.mapCameraPoints2Depth(bodies(1).Position');
                        %Matriz de movimiento x y z 3x25 de cada frame
            pos3D=bodies(1).Position;
            %Contador de Frame       
            i=i+1;
            %Matriz de movimiento total
            Q(i*4-3,:)=bodies.Orientation(1,:);
            Q(i*4-2,:)=bodies.Orientation(2,:);
            Q(i*4-1,:)=bodies.Orientation(3,:);
            Q(i*4,:)=bodies.Orientation(4,:);
            M(i*3-2,:)=pos3D(1,:);
            M(i*3-1,:)=pos3D(2,:);
            M(i*3,:)=pos3D(3,:);
        end
